...
To maintain a seamless user experience even in unexpected scenarios, a fully branded and functional 404 error page has been implemented.

Key Features:
Consistent Branding: The error page uses the same header, footer, fonts (Poppins), and design principles as the rest of the Jaura & Co website for visual consistency.

Dynamic Content with EJS: Error messages are populated using EJS variables (<%= title %>, <%= message %>, and <%= description %>) to allow flexible error rendering.

#### Helpful Navigation:

A clearly styled message and icon indicating the error.

Prominent buttons to return to the Home page or go directly to the Product Search page.

Fully responsive layout that works on all device sizes.

Font Awesome icons and semantic HTML improve accessibility and readability.
...
Routers, Auth, Scripts, Styling, Database, Images, Models, Test and view templates etc, are all separated into dedicated directories.
